...
val of_client_fd : ?buf_size:int -> Simple_httpd_domain.client ->
                   Unix.file_descr -> t
(** Allow a client to Make a buffered stream from the given descriptor.
    The call will be scheduled if read blocks and also regularly, accoring
    to the client's granularity *)

val of_client_fd_close_noerr : ?buf_size:int -> Simple_httpd_domain.client ->
                   Unix.file_descr -> t
(** Same as {!of_fd} but the [close] method will never fail. *)
